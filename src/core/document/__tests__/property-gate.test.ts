...
import { loadDocument, migrateV2ToV3, migrateV3ToV4, migrateV4ToV5 } from "../migrations";
  ...
  it("500 random v2 documents migrate to valid v3, keep every value, and round-trip unchanged", () => {
    fc.assert(
      fc.property(v2DocumentArb, (v2) => {
        // v2 → v3 is this gate's subject; v3 → v4 → v5 (Phases 7, 46) only add collections, so the result is checked as current data.
        const { document: v3Only, report } = migrateV2ToV3(v2);
        const v3 = migrateV4ToV5(migrateV3ToV4(v3Only).document).document;
