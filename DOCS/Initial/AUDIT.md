...
**Consumed By:** `DOCS/Initial/ROADMAP.md` v3.3.0. Every fix phase cites the audit IDs (`AUD-xx`) it closes. Section J (AUD-44 … AUD-55) was added on 2026-09-26 by the production-readiness review behind ROADMAP v3.0 (§4.2 there). AUD-56 was added the same day with ROADMAP v3.1, AUD-57 with v3.2, and AUD-58 with v3.3.
...
| AUD-36 | Open, in progress | **Phase 46 (awaiting CI)** adds the composition model: `main` and interaction compositions, layer time bars (in/out, stretch), precomps with time remap, markers, and golden-tested time maths (schema v5, decision 0002). It closes when Phase 46's gate passes in CI. |
...
| AUD-47 | Open, partly mitigated | The GATE-01 mitigation (GSAP is never in the editor) already holds. The routing half closes with 8.3, and the spec's §6/§7 tables are regenerated from the rule table (8.5). |
