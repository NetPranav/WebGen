...
| AUD-06 | Partial | The registry now links every archetype to its grammar type and derives legal states from it. `src/core/rules/` (Phase 8.1/8.2) now gives the UI a single `rules.canAdd`/`rules.validate` query surface operating on the real document, closing the "only exercised by tests" half. Wiring the editor's "+" menus to it (8.4) is the remaining piece — see decision 0005 §5. |
...
| AUD-45 | Open, spec written | The grammar-level fix is written: `lazylayout_element_grammer.md` v0.2 §13 (the Reactive category, 3.F effect surfaces, rules 6.9–6.16). The engine design is in `INTERACTIVE_EFFECTS_ENGINE_SPECIFICATION.md`. It closes when Phases 8, 59 and 60 pass their gates. |
| AUD-46 | Open, planned | Track X (61–66). |
| AUD-47 | Open, partly mitigated | The GATE-01 mitigation (GSAP is never in the editor) already holds. `src/core/rules/routing.ts` (8.3 Stage 1) now routes today's property catalogue CSS-first and can never return a GSAP backend by construction (`RouteBackend` has no `"gsap"` member); the full device-tier/GPU-budget decision tree needs Track X (59–61). The spec's §7 tables are corrected in place (8.5, decision 0005). |
| AUD-48 | Open, planned | Phase 64 (decision 0006) and the Phase 9 v3.0 amendment. |
...
| AUD-52 | Partial (8.5 reconciliation done) | The 5 named contradictions are resolved, grammar-wins, with a table-driven test (`src/core/rules/__tests__/reconciliation.test.ts`, decision 0005). The engine spec's §7 per-type tables and §13.5 are corrected in place. A ⚠️/🔒 conditional/subsumed tier the type system never modelled (Container's Press and 6 other cells) is added (`src/core/rules/conditional-gates.ts`), verified against grammar §9's full 320-cell matrix. Closes fully when 8's Verification Gate (the Playwright "illegal animation" test and the editor grep gate) is green — 8.4 (UI contract) is not started. |
