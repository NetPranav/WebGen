...
| AUD-32 | Partial (Phase 42.1) | One vocabulary now *exists*: `src/core/document/properties.ts` defines every canonical path (CONVENTIONS §4 names) and maps every flat v2 key and the `transform.translateX/Y` preset dialect onto it, per archetype where the meaning differs. Tests prove every preset track, archetype default and demo prop resolves. Stored documents and readers still use the old names; closes when 42.2's migration lands and the scrub regression test passes. |
